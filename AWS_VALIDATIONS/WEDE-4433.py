...
conn_s3_path = 's3://alt-we-tgt-dev-v1/we/dev/feature/conn/conn_mapping/delta/00000000'
all_social_s3_path = 's3://alt-we-tgt-dev-v1/we/dev/feature/conn/all_social/delta/00000000'


def match_conn_all_social(conn_s3_path, all_social_s3_path, source, type):
    ...
    # print("TOTAL NUMBER OF RECORDS AT all_social = ", all_social_s3_df.count())

    # Filter by source and type
    df2 = all_social_s3_df.filter((all_social_s3_df['source'] == source) & (all_social_s3_df['type'] == type))

    # print(f"All social count for source = {source} and type = {type} = {df2.count()}" )

    Objective: To
    validate if all
    records in df2 and present in df1 and that
    all
    the
    fileds(we_pid, link_id and type)
    match.
    if you could only verify that the following code meets with this objective.
...
# print(f"joined count for source {source} and type {type} = {joined_df.count()} and df2.count() = {df2.count()}")

# Assert that all rows in all_social match with conn_mapping data
assert joined_df.count() == df2.count(), f"FAILURE: Following all_social records not found in CONN_MAPPING: {df2_rows_not_in_df1.show(df2_rows_not_in_df1.count(), truncate=False)}"

print(f"SUCCESS: ALL SOCIAL RECORDS FOUND IN CONN_MAPPING for type = {type}")

match_conn_all_social(conn_s3_path, all_social_s3_path, 10, 1)
match_conn_all_social(conn_s3_path, all_social_s3_path, 10, 2)
match_conn_all_social(conn_s3_path, all_social_s3_path, 10, 3)
match_conn_all_social(conn_s3_path, all_social_s3_path, 10, 4)
match_conn_all_social(conn_s3_path, all_social_s3_path, 10, 11)
...
def not_match_conn_all_social(conn_s3_path, all_social_s3_path, source, type):
    ...
    logger.info(
        f"joined count for source {source} and type {type} = {joined_df.count()} and df2.count() = {df2.count()}")
